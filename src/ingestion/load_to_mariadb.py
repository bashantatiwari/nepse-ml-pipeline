...
def load_data():
    ...
    # Convert date to standard python date objects
    df["published_date"] = pd.to_datetime(df["published_date"]).dt.date
    
    # Drop rows without critical info
    df = df.dropna(subset=["published_date", "close"])
    
    # Replace NaN with None so MariaDB connector treats them as NULL
    ...
    try:
        with client.get_connection() as conn:
            ...
            # Fetch existing dates to handle duplicates in Python
            ...
            # Standard INSERT query (ColumnStore safe)
            insert_query = """
            ...
            conn.commit()
            logger.info(f"Rows successfully inserted: {cursor.rowcount}")
